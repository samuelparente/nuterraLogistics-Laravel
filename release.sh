...
git commit -m "$MENSAGEM_COMMIT" || echo "⚠️  Sem alterações para commitar ou erro ao commitar. A continuar..."
...
if [[ "$MERGE_DEVELOP" =~ ^[sS]$ ]]; then
    ...
    echo "🔧 A fazer merge..."
    if ! git merge --no-ff "$BRANCH_ATUAL" -m "Merge de $BRANCH_ATUAL para develop (versão $VERSAO)"; then
...
if [[ "$MERGE_MAIN" =~ ^[sS]$ ]]; then
    ...
    echo "🔧 A fazer merge..."
    if ! git merge --no-ff develop -m "Release $VERSAO - Merge de develop para main"; then
        echo
        echo "⚠️  Ocorreu um problema durante o merge para main."
